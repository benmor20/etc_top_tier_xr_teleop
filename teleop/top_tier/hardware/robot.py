...
=======
        ...
        self._joints = self._create_joints()
        for joint_type, joint in self._joints.items():
            assert joint.joint_type == joint_type, f"{self.__name__}._create_joints made a {joint.joint_type} joint mapped as a {joint_type}"

    @staticmethod
    @abstractmethod
    def _create_joints() -> dict[JointType, Joint]:
        """
        Create the set of joints that are used to control this robot

        Returns:
            the set of joints used on this robot, one for each JointType on this robot
        """
        pass
>>>>>>> Stashed changes
